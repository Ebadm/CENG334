...
int execute_bundles(vector<b_commands> all_bundles, parsed_input * parsing, int ind){
    ...
    for (i = 0; i < bundle_count; i++) {                    //go over all exec bundles
        ...
        if (i == 0){        //first bundle
            ...
            for (int j = 0; j < b_PROCESS_NUM; j++) {       //run processes of a sucessor bundle
                ...
                if (pids[j] == 0) {
                    ...
                    close(pipes[i-1][1]);
                   cout << "Second Pipe Called - " << i-1 << j << second_pipes[i-1][j][0] << endl;
        ...
        if ( i+1 < bundle_count){       //check if not last bundle or not
            ...
            for (int j = 0; j < nextb_PROCESS_NUM; j++) { //run processes of a sucessor bundle
                ...
                //close(second_pipes[i-1][j][0]);
                close(pipes[i][1]);
                cout << "Second Pipe - " << i << j << second_pipes[i][j][0] << endl;
                dup2(pipes[i][0], second_pipes[i][j][1]);
                //dup2(pipes[i][0], second_pipes[i][j][0]);      //read from all proceses
                close(pipes[i][0]);
                //execlp("cat", "cat", NULL);
                return 0;
                }
                ...
                /*for (int j = 0; j < nextb_PROCESS_NUM; j++) {
                    close(pipes[i][1]);
                    dup2(repeater_pipe[i][1], second_pipes[i][j][0]);      //read from all proceses
                    close(pipes[i][0]);
                }*/
                //fork to send data?
    ...
    for (int z = 0; z < bundle_count; z++) {            //close Pipes
        close(pipes[z][0]);
        close(pipes[z][1]);
    }
